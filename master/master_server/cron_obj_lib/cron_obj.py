...
class CronObj:
    ...
    def __init__(self, sid, pre_tables, result_tables, cron, api, server_id):
        ...
        # 记录信息的hash值用于之后的比较
        self.hash_ack = get_hash(info_dict_new)
    ...
    def refresh(self, sid, pre_tables, result_tables, cron, api, server_id):
        if sid == self.sid:
            info_dict_new = {'sid': sid, 'result_tables': result_tables, 'pre_tables': pre_tables,
                             'cron': cron, 'api': api}
            hash_ack_new = get_hash(info_dict_new)
    ...
    def callback(self, ch, method, properties, body):
        ...
        if 'sid' in log_dict and log_dict['sid'] == self.sid:
            logging = WriteLog(name="events_listener".format(sid=self.sid))
        ...
        if cron_tree_status == 0 and status == 1:
            self.running_start_event(hash_id, subversion)
    ...
    def running_start_event(self, hash_id, subversion):
        self.cron_tree_obj.running_start(event_hash_id=hash_id, subversion=subversion)
